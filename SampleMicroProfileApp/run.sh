...
RUNNING=$(docker inspect --format="{{ .State.Running }}" SampleMicroProfileApp 2> /dev/null)
if [ $? -eq 1 ] || [ "$RUNNING" == "false" ]; then
    ...
    echo "Stopping... with title SampleMicroProfileApp"
    docker rm -f SampleMicroProfileApp
fi
